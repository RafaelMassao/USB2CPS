...
static void cmd_profile_save(const char* json)
{
    ...
    // Save immediately so the last saved custom profile survives a power cut.
...
static void cmd_profile_delete(const char* json)
{
    ...
    // Save immediately so profile deletion is durable before power is removed.
    flash_save_now(settings);
    send_ok();
}
...
static void cmd_profile_clone(const char* json)
{
    ...
    // Save immediately so the cloned custom profile survives a power cut.
    flash_save_now(settings);
