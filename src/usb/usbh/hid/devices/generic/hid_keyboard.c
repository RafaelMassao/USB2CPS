...
void process_hid_keyboard(uint8_t dev_addr, uint8_t instance, uint8_t const* hid_kb_report, uint16_t len)
{
  ...
  for(uint8_t i=0; i<6; i++)
  {
    if ( report->keycode[i] )
    {
      ...
      // B1 slot
      if (report->keycode[i] == HID_KEY_Y ||
          report->keycode[i] == HID_KEY_B ||
          report->keycode[i] == HID_KEY_P ||
          report->keycode[i] == HID_KEY_KEYPAD_7 ||
          report->keycode[i] == HID_KEY_KEYPAD_3) btns_b1 = true;
      // B2 slot
      if (report->keycode[i] == HID_KEY_U ||
          report->keycode[i] == HID_KEY_N ||
          report->keycode[i] == HID_KEY_PERIOD ||
          report->keycode[i] == HID_KEY_KEYPAD_8) btns_b2 = true;
      // B3 slot
      if (report->keycode[i] == HID_KEY_I ||
          report->keycode[i] == HID_KEY_M ||
          report->keycode[i] == HID_KEY_SLASH || //ACHAR A TECLA ;/:
          report->keycode[i] == HID_KEY_KEYPAD_9) btns_b3 = true;
      // B4 slot
      if (report->keycode[i] == HID_KEY_G ||
          report->keycode[i] == HID_KEY_K ||
          report->keycode[i] == HID_KEY_BRACKET_LEFT ||
          report->keycode[i] == HID_KEY_KEYPAD_4) btns_b4 = true;
          
      // L1 slot
      if (report->keycode[i] == HID_KEY_H ||
          report->keycode[i] == HID_KEY_APOSTROPHE ||
          report->keycode[i] == HID_KEY_KEYPAD_5) btns_l1 = true;
      // R1 slot
      if (report->keycode[i] == HID_KEY_J ||
          report->keycode[i] == HID_KEY_SEMICOLON || //ACHAR O NOME HID PARA Ç
          report->keycode[i] == HID_KEY_KEYPAD_6) btns_r1 = true;
      // L2 slot
      if (report->keycode[i] == HID_KEY_O ||
           report->keycode[i] == HID_KEY_COMMA || //DIZ QUE É ,/<
          report->keycode[i] == HID_KEY_KEYPAD_1) btns_l2 = true;
      // R2 slot
      if (report->keycode[i] == HID_KEY_L ||
          report->keycode[i] == HID_KEY_V ||
          report->keycode[i] == HID_KEY_KEYPAD_2) btns_r2 = true;
