...
void process_hid_keyboard(uint8_t dev_addr, uint8_t instance, uint8_t const* hid_kb_report, uint16_t len)
{
  ...
  for(uint8_t i=0; i<6; i++)
  {
    if ( report->keycode[i] )
    {
      // ===== START / COIN (system buttons) =====
      // Start: ESC, Numpad *
      if (report->keycode[i] == HID_KEY_ESCAPE ||
          report->keycode[i] == HID_KEY_KEYPAD_MULTIPLY) btns_run = true;
      // Coin/Select: F1, Numpad /
      if (report->keycode[i] == HID_KEY_F1 ||
          report->keycode[i] == HID_KEY_KEYPAD_DIVIDE) btns_sel = true;;

      // ===== ACTION BUTTONS =====
      // Each slot has a primary key and optional alternates.
      // All keys in the same slot are equivalent — the custom mapping
      // system will remap whichever is pressed during recording.
      //
      // ABNT2-safe layout with conflict-free slots.
      // B1: U, G, M, Z, Space, Enter, 5, Numpad 7, F2
      // B2: I, H, Ç(;), X, Tab, Backspace, 6, Numpad 8, F3
      // B3: O, V, ,/< (includes HID_KEY_EUROPE_2), C, 7, Numpad 9, F4
      // B4: J, ~/^ (ABNT2 -> HID_KEY_APOSTROPHE), 8, Numpad 4, F5
      // L1: K, B, Q, 9, Numpad 5, F6
      // R1: L, R, 0, Numpad 6, F7
      // L2: P, N, Y, F, PageUp, Numpad 1, F8
      // R2: ´/` (ABNT2 -> HID_KEY_BRACKET_LEFT), T, PageDown, Numpad 2, F9

      // B1 slot
      if (report->keycode[i] == HID_KEY_U ||
          report->keycode[i] == HID_KEY_PERIOD ||
          report->keycode[i] == HID_KEY_J ||
          report->keycode[i] == HID_KEY_G ||
          report->keycode[i] == HID_KEY_ENTER ||
          report->keycode[i] == HID_KEY_Z ||
          report->keycode[i] == HID_KEY_SPACE ||
          report->keycode[i] == HID_KEY_KEYPAD_7 ||
          report->keycode[i] == HID_KEY_F2 ||
          report->keycode[i] == HID_KEY_5) btns_b1 = true;
      // B2 slot
      if (report->keycode[i] == HID_KEY_I ||
          report->keycode[i] == HID_KEY_M |
          report->keycode[i] == HID_KEY_H ||
          report->keycode[i] == HID_KEY_SEMICOLON ||
          report->keycode[i] == HID_KEY_BACKSPACE ||
          report->keycode[i] == HID_KEY_X ||
          report->keycode[i] == HID_KEY_TAB ||
          report->keycode[i] == HID_KEY_KEYPAD_8 ||
          report->keycode[i] == HID_KEY_F3 ||
          report->keycode[i] == HID_KEY_6) btns_b2 = true;
      // B3 slot
      if (report->keycode[i] == HID_KEY_O ||
          report->keycode[i] == HID_KEY_V ||
          report->keycode[i] == HID_KEY_COMMA ||
          report->keycode[i] == HID_KEY_EUROPE_2 ||
          report->keycode[i] == HID_KEY_C ||
          report->keycode[i] == HID_KEY_KEYPAD_9 ||
          report->keycode[i] == HID_KEY_KEYPAD_3 ||
          report->keycode[i] == HID_KEY_F4 ||
          report->keycode[i] == HID_KEY_7) btns_b3 = true;
      // B4 slot
      if (report->keycode[i] == HID_KEY_O ||
          report->keycode[i] == HID_KEY_APOSTROPHE ||
          report->keycode[i] == HID_KEY_V ||
          report->keycode[i] == HID_KEY_COMMA ||
          report->keycode[i] == HID_KEY_EUROPE_2 ||
          report->keycode[i] == HID_KEY_KEYPAD_4 ||
          report->keycode[i] == HID_KEY_F5 ||
          report->keycode[i] == HID_KEY_8) btns_b4 = true;
          
      // L1 slot
      if (report->keycode[i] == HID_KEY_K ||
          report->keycode[i] == HID_KEY_B ||
          report->keycode[i] == HID_KEY_Q ||
          report->keycode[i] == HID_KEY_9 ||
          report->keycode[i] == HID_KEY_KEYPAD_5 ||
          report->keycode[i] == HID_KEY_F6) btns_l1 = true;
      // R1 slot
      if (report->keycode[i] == HID_KEY_L ||
          report->keycode[i] == HID_KEY_R ||
          report->keycode[i] == HID_KEY_0 ||
          report->keycode[i] == HID_KEY_KEYPAD_6 ||
          report->keycode[i] == HID_KEY_F7) btns_r1 = true;
      // L2 slot
      if (report->keycode[i] == HID_KEY_P ||
          report->keycode[i] == HID_KEY_N ||
          report->keycode[i] == HID_KEY_Y ||
          report->keycode[i] == HID_KEY_F ||
          report->keycode[i] == HID_KEY_PAGE_UP ||
          report->keycode[i] == HID_KEY_R ||
          report->keycode[i] == HID_KEY_KEYPAD_1 ||
          report->keycode[i] == HID_KEY_F8) btns_l2 = true;
      // R2 slot
      if (report->keycode[i] == HID_KEY_BRACKET_LEFT ||
          report->keycode[i] == HID_KEY_EQUAL ||
          report->keycode[i] == HID_KEY_PAGE_DOWN ||
          report->keycode[i] == HID_KEY_T ||
          report->keycode[i] == HID_KEY_KEYPAD_2 ||
          report->keycode[i] == HID_KEY_F9) btns_r2 = true;

      // L3/R3 (auxiliary)
      // Keep aux buttons on dedicated keys only to avoid conflicts with ABNT2 action slots.
      if (report->keycode[i] == HID_KEY_F10) btns_l3 = true;
      if (report->keycode[i] == HID_KEY_F11) btns_r3 = true;
      // HAT SWITCH
