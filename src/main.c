...
static void __not_in_flash_func(core0_main)(void)
{
  ...
  uint32_t last_players_us = time_us_32();
  uint32_t last_leds_us = last_players_us;
  uint32_t last_storage_us = last_players_us;

  // Run non-critical housekeeping tasks on a cadence so the hot input/output
  // path is serviced as often as possible.
  const uint32_t players_period_us = 1000;   // 1 kHz
  const uint32_t leds_period_us = 1000;      // 1 kHz
  const uint32_t storage_period_us = 10000;  // 100 Hz

  while (1)
  {
    // Poll all input interfaces first to minimize HID->GPIO latency.
    ...
    if ((uint32_t)(now_us - last_players_us) >= players_period_us) {
      if (first_loop) printf("[joypad] Loop: players\n");
      players_task();
      last_players_us = now_us;
    }

    if ((uint32_t)(now_us - last_leds_us) >= leds_period_us) {
      if (first_loop) printf("[joypad] Loop: leds\n");
      leds_task();
      last_leds_us = now_us;
    }

    if ((uint32_t)(now_us - last_storage_us) >= storage_period_us) {
      if (first_loop) printf("[joypad] Loop: storage\n");
      storage_task();
      last_storage_us = now_us;
    }
