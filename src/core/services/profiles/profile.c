...
void profile_save_to_flash(output_target_t output)
{
    ...
    // Preserve the full settings page when updating only the active profile.
    // Saving a partially initialized flash_t here can erase custom profiles on
    // the next boot because flash_t stores the active index and profile data
    // together in the same 256-byte journal entry.
