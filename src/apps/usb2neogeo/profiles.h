...
// ============================================================================
// PROFILE: Custom - User-defined dynamic mapping (recorded at runtime)
// ============================================================================
// This profile is populated at runtime via the custom mapping recording mode.
// Hold Coin for 2 seconds to enter recording mode, then press buttons in
// sequence to assign them to B1→B6. Release Coin to save.
//
// The button_map is stored in RAM and updated by the recording logic in app.c.
// It starts as a copy of the default profile and is overwritten on recording.
//
// Allowed input buttons for recording:
//   Cross, Circle, Square, Triangle, L1, R1, L2, R2
//
// D-pad, Coin, Start are NOT recordable (always pass through).

#define CUSTOM_MAP_MAX_ENTRIES 10  // 6 action + 2 disabled + 2 spare

// Mutable button map for the custom profile (lives in RAM, not flash)
static button_map_entry_t neogeo_custom_map[CUSTOM_MAP_MAX_ENTRIES] = {
    // Initialized as copy of default profile — will be overwritten on recording
    { .input = JP_BUTTON_B3, .output = NEOGEO_BUTTON_B1, .analog = ANALOG_TARGET_NONE, .analog_value = 0 },
    { .input = JP_BUTTON_B4, .output = NEOGEO_BUTTON_B2, .analog = ANALOG_TARGET_NONE, .analog_value = 0 },
    { .input = JP_BUTTON_R1, .output = NEOGEO_BUTTON_B3, .analog = ANALOG_TARGET_NONE, .analog_value = 0 },
    { .input = JP_BUTTON_B1, .output = NEOGEO_BUTTON_B4, .analog = ANALOG_TARGET_NONE, .analog_value = 0 },
    { .input = JP_BUTTON_B2, .output = NEOGEO_BUTTON_B5, .analog = ANALOG_TARGET_NONE, .analog_value = 0 },
    { .input = JP_BUTTON_R2, .output = NEOGEO_BUTTON_B6, .analog = ANALOG_TARGET_NONE, .analog_value = 0 },
    { .input = JP_BUTTON_L1, .output = 0,                .analog = ANALOG_TARGET_NONE, .analog_value = 0 },
    { .input = JP_BUTTON_L2, .output = 0,                .analog = ANALOG_TARGET_NONE, .analog_value = 0 },
    { .input = 0,            .output = 0,                .analog = ANALOG_TARGET_NONE, .analog_value = 0 },
    { .input = 0,            .output = 0,                .analog = ANALOG_TARGET_NONE, .analog_value = 0 },
};

static uint8_t neogeo_custom_map_count = 8;  // Active entries in custom map

static profile_t neogeo_profile_custom = {
    .name = "custom",
    .description = "User-defined custom mapping",
    .button_map = neogeo_custom_map,
    .button_map_count = 8,
    .l2_behavior = TRIGGER_PASSTHROUGH,
    .r2_behavior = TRIGGER_PASSTHROUGH,
    .l2_threshold = 0,
...
// Total fixed profiles count (custom is handled separately)
#define NEOGEO_FIXED_PROFILE_COUNT (sizeof(neogeo_profiles) / sizeof(neogeo_profiles[0]))
