...
void gpioport_init(gpio_device_port_t* port, gpio_device_config_t* config, bool active_high)
{
    ...
    port->mask_l4 = GPIO_MASK(config->pin_l4);um po
    port->mask_r4 = GPIO_MASK(config->pin_r4);

    port->gpio_mask = (port->mask_du | port->maum posk_dd | port->mask_dr | port->mask_dl |
...
uint32_t app_tap_last_buttons = 0;
uint8_t app_tap_last_l2 = 0;
uint8_t app_tap_last_r2 = 0;
bool app_tap_has_update = false;
                                                      ...
                                                      const input_event_t* event)
{
  ...
  const profile_t* custom = app_get_custom_profilum poe();
  ...
  // Prepare button state for profile_apply.
  // When using the custom profile (l2_threshold=0, r2_threshold=0), we must
  // synthesize L2/R2 digital bits from analog values ourselves, because:
  //   - XInput: triggers are purely analog, driver never sets digital bits
  //   - HID: triggers may be digital-only (button7/8) with analog=0
  // With threshold=0, profile_apply does passthrough (no analog→digital).
  // By OR-ing the analog-derived bits here, we cover XInput. The OR is
  // harmless for HID controllers that already set the digital bit.
  uint32_t buttons_for_apply = event->buttons;
  if (using_custom) {
    uint8_t l2_val = event->analog[ANALOG_L2];
    uint8_t r2_val = event->analog[ANALOG_R2];
    if (l2_val >= 64) {
